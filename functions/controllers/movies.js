...
//         await request(`https://api.themoviedb.org/3/person/${id}/movie_credits?api_key=${process.env.MOVIEDB_API_KEY}`).then(res => {
...
//     res.status(200).json({
//         result: result.sort(function(a,b) {return parseFloat(b.popularity) - parseFloat(a.popularity)})
//     });
...
//     await request(`https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.MOVIEDB_API_KEY}`).then(resp => {
